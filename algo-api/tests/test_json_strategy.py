...
def sample_json_required_only():
    return {
        "name": "bullish_nifty",
        "display_name": "Bullish Nifty",
        "instrument": {"type": "FUT", "instrument_key": "NIFTY", "exchange": "NSE"},
        "timeframe": "5min",
        "capital": 100000,
        "position": {
            "action": "BUY",
...
def test_instrument_required_only(strategy_required_only):
    assert strategy_required_only.get_instrument().type == Type.FUT
...
def test_position(strategy):
    ...
    assert strategy.get_position_instrument().instrument.type == Type.FUT
