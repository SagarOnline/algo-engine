...
def test_indicator_atr_with_dataframe():
    ...
    params = {"period": 14}
    result = indicator_atr(df, params)
    assert isinstance(result, float)

def test_indicator_atr_with_list_of_dicts():
    data = [
        {"high": 10, "low": 5, "close": 7},
        {"high": 12, "low": 6, "close": 9},
        {"high": 14, "low": 7, "close": 11},
    ...
    params = {"period": 14}
    result = indicator_atr(data, params)
    assert isinstance(result, float)

def test_indicator_atr_with_no_historical_data():
    df = pd.DataFrame([])
    params = {"period": 14}
    with pytest.raises(RuntimeError, match="historical_data is empty in atr indicator"):
        indicator_atr(df, params)

def test_indicator_atr_with_missing_columns():
    df = pd.DataFrame([
        {"high": 10, "low": 5},
        {"high": 12, "low": 6},
    ])
    params = {"period": 14}
    with pytest.raises(ValueError, match="Missing required column 'close' in historical_data"):
        indicator_atr(df, params)

def test_indicator_atr_with_default_params():
    df = pd.DataFrame([
        {"high": 10, "low": 5, "close": 7},
        {"high": 12, "low": 6, "close": 9},
        {"high": 14, "low": 7, "close": 11},
