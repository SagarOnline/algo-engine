...
from algo.domain.strategy.strategy import Instrument,Exchange,Expiring, Expiry, PositionInstrument, TradeAction, Strategy, Type
...
class DummyRepository:
    def list_strategies(self):
        return [
            DummyStrategy("strat1", "Strategy One", "Strategy One", Instrument(type=Type.FUT, exchange=Exchange.NSE, instrument_key="NIFTY23JUNFUT")),
            DummyStrategy("strat2", "Strategy Two", "Strategy Two", Instrument(type=Type.FUT, exchange=Exchange.NSE, instrument_key="NIFTY23JUNFUT")),
        ]

def test_strategy_dto_to_dict():
    instr = Instrument(type=Type.FUT, exchange=Exchange.NSE, instrument_key="NIFTY23JUNFUT", expiry=Expiry.MONTHLY, expiring=Expiring.CURRENT)
