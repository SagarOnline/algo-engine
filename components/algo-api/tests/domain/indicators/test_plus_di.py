...
def test_indicator_plus_di_with_dataframe():
    ...
    params = {"period": 14}
    result = indicator_plus_di(df, params)
    assert isinstance(result, float)

def test_indicator_plus_di_with_list_of_dicts():
    data = [
        {"high": 10, "low": 5, "close": 7},
        {"high": 12, "low": 6, "close": 9},
        {"high": 14, "low": 7, "close": 11},
    ...
    params = {"period": 14}
    result = indicator_plus_di(data, params)
    assert isinstance(result, float)

def test_indicator_plus_di_with_no_historical_data():
    df = pd.DataFrame([])
    params = {"period": 14}
    with pytest.raises(RuntimeError, match="historical_data is empty in plus_di indicator"):
        indicator_plus_di(df, params)

def test_indicator_plus_di_with_default_params():
    df = pd.DataFrame([
        {"high": 10, "low": 5, "close": 7},
        {"high": 12, "low": 6, "close": 9},
        {"high": 14, "low": 7, "close": 11},
